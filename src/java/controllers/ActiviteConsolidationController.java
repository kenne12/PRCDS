...
public class ActiviteConsolidationController {
    ...
    @EJB
    private TypeactiviteFacadeLocal typeactiviteRegionFacadeLocal;
    private Typeactivite typeactivite = new Typeactivite();
    private List<Typeactivite> typeactiviteRegions = new ArrayList<>();
    ...
    private void init() {
        ...
        try {
            if (!axes.isEmpty()) {
                ...
                List<ProblemeRegion> pbs = problemeRegionFacadeLocal.findByAxeRegion(axe, SessionMBean.getRegion(), 1);
                if (pbs.isEmpty()) {
                    pourcentageAxe = 0d;
                } else {
                    Integer conteur = 0;
                ...
                if (!sousaxes.isEmpty()) {
                    ...
                    activiteRegions = activiteRegionFacadeLocal.find(SessionMBean.getRegion(), sousaxe, 1);

                    List<ProblemeRegion> pbs1 = problemeRegionFacadeLocal.findBySousAxeRegion(sousaxe, SessionMBean.getRegion(), 1);
                    if (!pbs1.isEmpty()) {
    ...
    public void updateProbleme() {
        try {
            ...
            if (indicateur.getIdindicateur() != null) {
                ...
                if (sousaxe != null) {
                    problemeRegions = problemeRegionFacadeLocal.find(indicateur, SessionMBean.getRegion(), 1);

                    if (!problemeRegions.isEmpty()) {
                        problemeRegion = problemeRegions.get(0);
                    } else {
                        JsfUtil.addErrorMessage("L'indicateur est non evalué pour la region");
                    }
                }
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (!indicateurs.isEmpty()) {
                for (Indicateur i : indicateurs) {
                    List<IndicateurRegion> ids = indicateurRegionFacadeLocal.findByRegionIndicateurObservation(SessionMBean.getRegion(), i, 1);
                    if (!ids.isEmpty()) {
        ...
        try {
            if (!sousaxes.isEmpty()) {
                ...
                if (sousaxe != null) {
                    ...
                    List<ProblemeRegion> pbs = problemeRegionFacadeLocal.findBySousAxeRegion(sousaxe, SessionMBean.getRegion(), 1);
        ...
        try {
            if ("Edit".equals(mode)) {
                ...
                if (activite1.getCoutunitaire() != activiteRegion.getCoutunitaire()) {
                    List<Structure> structures = structureFacadeLocal.find(SessionMBean.getRegion().getIdregion(), typestructure);
                    if (!structures.isEmpty()) {
                        ...
                        for (ActiviteStructureRegion a : activiteStructureRegionFacadeLocal.find(activiteRegion)) {
                            a.setCout(activiteRegion.getCoutunitaire());
                ...
                if (!structures.isEmpty()) {
                    ...
                    for (Structure s : structures) {
                        for (Annee a : annees) {
                            ActiviteStructureRegion activiteStructureRegion = new ActiviteStructureRegion();
                            activiteStructureRegion.setIdactiviteStructureRegion(activiteStructureRegionFacadeLocal.nextId());
                            activiteStructureRegion.setIdstructure(s);
                            activiteStructureRegion.setIdannee(a);
                            activiteStructureRegion.setIdactiviteRegion(activiteRegion);
                            activiteStructureRegion.setPrograme(false);
                            activiteStructureRegion.setCout(activiteRegion.getCoutunitaire());
                            activiteStructureRegionFacadeLocal.create(activiteStructureRegion);
                        }
                    }
                }
        ...
        try {
            ...
            if ("Edit".equals(mode)) {
                if (activiteRegion != null) {
                    ...
                    if (activiteRegion.getIdsourcefi() != null) {
    ...
    public List<ProblemeRegion> getProblemes() {
        return problemeRegions;
    }

    public void setProblemes(List<ProblemeRegion> problemeRegions) {
        this.problemeRegions = problemeRegions;
    }
    ...
    public List<Typeactivite> getTypeactiviteRegions() {
        typeactiviteRegions = typeactiviteRegionFacadeLocal.findAllRange();
        return typeactiviteRegions;
    }

    public void setTypeactiviteRegions(List<Typeactivite> typeactiviteRegions) {
        this.typeactiviteRegions = typeactiviteRegions;
    }
    ...
    public ActiviteRegionElementCout getActiviteRegionElementCout() {
        return activiteRegionElementCout;
    }

    public void setActiviteRegionElementCout(ActiviteRegionElementCout activiteRegionElementCout) {
        this.activiteRegionElementCout = activiteRegionElementCout;
    }
